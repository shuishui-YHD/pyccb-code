...
def ccb_to_text(ccb_code):
    ...
    while i < len(ccb_code):
        ...
        if len(ccb_char) != 16:
            break
        binary = ccb_char.replace('9', '0').replace('1', '1')
...
def compile_and_run_ccb(ccb_code):
    ...
    try:
        ...
        messagebox.showerror("错误", f"编译或运行91py代码时出错: {e}")

class CBCompilerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("91py语言编译器")
